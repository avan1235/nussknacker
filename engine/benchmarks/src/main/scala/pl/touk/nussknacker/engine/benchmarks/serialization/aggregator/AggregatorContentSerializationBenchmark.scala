...
@State(Scope.Thread)
//In this benchmark we simulate different methods of serialization of FlinkRangeMap used in AggregatorFunction
class AggregatorContentSerializationBenchmark {

  private def treeMap(content: (Long,AnyRef)*) = new SerializationBenchmarkSetup(
    TypeInformation.of(classOf[TreeMap[Long, AnyRef]]), TreeMap(content:_*)
  )

  private def treeMapOpt(content: (Long,AnyRef)*) = new SerializationBenchmarkSetup(
    treeMapTypeDef(implicitly[TypeInformation[Map[String, AnyRef]]]).asInstanceOf[TypeInformation[TreeMap[Long, AnyRef]]], TreeMap(content:_*)
  )
  ...
  private val manyBucketsOpt = new SerializationBenchmarkSetup(
  ...
  private val manyBucketsSomeOpt = new SerializationBenchmarkSetup(
  ...
  private val manyBucketsOptKryoValues = new SerializationBenchmarkSetup(
  ...
  private val manyBucketsOptKryoScalaMapValues = new SerializationBenchmarkSetup(
