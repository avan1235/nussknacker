...
class ConfluentAvroToJsonFormatterFactory(schemaRegistryClientFactory: ConfluentSchemaRegistryClientFactory, kafkaConfig: KafkaConfig) extends RecordFormatterFactory {
  override def create[K, V](deserializationSchema: KafkaDeserializationSchema[ConsumerRecord[K, V]]): RecordFormatter = {
    ConfluentAvroToJsonFormatter(schemaRegistryClientFactory, kafkaConfig, formatKey = true)
  }
}